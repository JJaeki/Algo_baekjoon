...
public class 골드4_1633_최고의팀만들기 {
    ...
    public static int solution(int i, int wIndex, int bIndex, int N) {
        ...
        //white
        if (wIndex < 15) {
            ans = Math.max(ans, solution(i + 1, wIndex + 1, bIndex, N) + white[i]);
        }
        if (bIndex < 15) {
            ans = Math.max(ans, solution(i + 1, wIndex, bIndex + 1, N) + black[i]);
        }
        //black
